...
class Student {
	...
	function find_all_students($name=""){
		...
		static function studentAuthentication($username,$password){
		global $mydb;
		$mydb->setQuery("SELECT * FROM `tblstudent` WHERE `username` = '". $username ."' and `password` = '". $password ."'");
		$cur = $mydb->executeQuery();
		if($cur==false){
			die(mysqli_error($cur));
		}
	...
	function find_pass($id="",$password=""){
		global $mydb;
		$mydb->setQuery("SELECT * FROM ".self::$tblname." 
